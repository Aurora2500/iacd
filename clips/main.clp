...
(defrule phone-to-person
    (declare (salience 10))
    ?i <- (phone_to_lookup ?t)
    (person (name ?n) (surname ?s) (cellphone ?t))
    =>
    (printout t "El número de móvil " ?t " pertenece a " ?n " " ?s crlf)
    (retract ?i)
)

; ***** phone to location people *****

(defrule phone_to_location_people
    (declare (salience 5))
    ?i <- (phone_to_lookup ?tloc)
    (location (name ?l) (telephone ?tloc))
    =>
    (printout t "El teléfono " ?tloc " pertenece a " ?l crlf)
    (printout t "Las personas que están en " ?l " son: " crlf)
    (do-for-all-facts ((?p person)) ; Loop
        (eq ?p:location ?l) ; Condition
        (printout t " * " ?p:name " " ?p:surname crlf) ; Action
    )
    (retract ?i)
)
